# Installs the sm_dark_full, sm_powershell, sm_dark_black_white, sm_matrix, sm_light, sm_amber and sm_ubuntu themes for VS Code.
...
try {
    Write-Host 'Downloading sm_dark_full, sm_powershell, sm_dark_black_white, sm_matrix, sm_light, sm_amber and sm_ubuntu...'
    Invoke-WebRequest -Uri $vsixUrl -OutFile $vsix -UseBasicParsing
    code --install-extension $vsix --force
    if ($LASTEXITCODE -ne 0) { throw 'The installation failed.' }

    Write-Host 'Restart VS Code, press Ctrl+K Ctrl+T and select "sm_dark_full", "sm_powershell", "sm_dark_black_white", "sm_matrix", "sm_light", "sm_amber" or "sm_ubuntu".'
}
finally {
